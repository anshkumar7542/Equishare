...
## Critical Pivot - Current Assignment
- Status: assignment scope changed. Do not continue treating this as a generic Splitwise clone.
- New product: shared expenses app for flatmates Aisha, Rohan, Priya, Meera, Dev, and Sam.
- Primary project risk: exact import of `expenses_export.csv`, anomaly detection, date-aware membership, currency handling, and explainable balances.
- Required input file: `expenses_export.csv`.
- Current workspace finding: `expenses_export.csv` is not present in `D:\Spreetail` as of this update, so exact CSV import implementation and anomaly verification are blocked until the file is provided.
- Existing code must not be deleted unless necessary. Reuse working authentication, relational database setup, split calculation, settlement, and balance foundations where possible.
- Required final deliverables are now:
  - `README.md`
  - `SCOPE.md` containing anomaly log and database schema
  - `DECISIONS.md` containing decision log
  - Import report generated by the app
  - `AI_USAGE.md` with AI tools, key prompts, and at least 3 cases where AI was wrong and how it was fixed
  - GitHub repo with meaningful commit history
  - Public deployed app URL

## Pivot Requirements
- Login module.
- Groups with membership changing over time:
  - Meera moved out at the end of March.
  - Sam moved in mid-April.
  - Dev joined for a trip.
...
- Import `expenses_export.csv` exactly as provided. No manual CSV editing allowed.
- Importer must detect at least 12 deliberate data problems.
- For every anomaly:
  - Detect it.
...
- Use relational DB only.

## Pivot Reuse And Change Assessment
- Reusable with limited changes:
  - Authentication module: signup/login/logout/protected routes can remain.
  - PostgreSQL + Prisma relational database foundation can remain.
  - Existing split-calculation service can be reused after adapting imported CSV rows into normalized split rows.
  - Existing balance service can be reused after adding membership date ranges, import anomaly policy handling, and currency-aware amounts.
  - Existing settlement model and UI can remain with possible group/person constraints.
  - Existing dashboard/group pages can be adapted to the flatmates/import workflow.
- Must change or add:
  - Add dated membership model fields or new membership-period table to represent Meera, Sam, and Dev timeline.
  - Add CSV import module, parser, staging tables, anomaly detection, import status UI, and generated import report.
  - Add anomaly table/model to persist detected problems, severity, policy, row number, field, raw value, and resolution status.
  - Add import report page/download endpoint generated by the app.
  - Add currency handling based on actual CSV contents; policy must be documented after inspecting the file.
  - Add explainable balance output that can show how imported rows, anomalies, memberships, and settlements affect balances.
  - Update seed/demo flow to use flatmates Aisha, Rohan, Priya, Meera, Dev, and Sam.
  - Update required docs to `SCOPE.md`, `DECISIONS.md`, and `AI_USAGE.md`.
- Existing generic features that may become secondary:
  - Manual expense creation/edit/delete remains useful, but CSV import becomes the priority.
  - Expense chat is no longer listed as a critical requirement in the pivot and should not displace import/anomaly work.

## Pivot Blockers
- `expenses_export.csv` is missing from the workspace, so exact schema/column mapping, actual split types present, actual currencies, and the 12 deliberate data problems cannot be verified yet.
- Public deployed app URL requires an external hosting provider and hosted PostgreSQL.
- GitHub meaningful commit history requires committing changes incrementally; current repository status must be reviewed before committing.

## CSV Import System Implementation
- Status: implemented for preview, anomaly detection, user actions, finalize, and report generation.
- Files created or changed:
  - `server/src/modules/imports/import.service.ts`: CSV parsing, row normalization, anomaly detection, duplicate detection, mixed date format detection, import finalization, report creation.
  - `server/src/modules/imports/imports.routes.ts`: authenticated import API routes.
  - `server/src/app.ts`: registers `/api/imports` and increases JSON body limit to 10 MB for CSV text upload.
  - `client/src/features/imports/ImportPage.tsx`: upload, preview, anomaly action, finalize, and report UI.
  - `client/src/app/router.tsx`: adds protected `/imports` route.
  - `client/src/app/AppLayout.tsx`: adds Import navigation item.
  - `client/src/types/domain.ts`: frontend import types.
  - `prisma/schema.prisma`: adds import models, anomaly enums, membership date fields, and currency fields.
  - `prisma/migrations/20260614010000_import_system/migration.sql`: relational migration for the import system.
- Upload approach:
  - The browser file input accepts `.csv` files.
  - The frontend reads the selected file with `File.text()` and sends `{ groupId, fileName, csvText }` to the backend.
  - No manual CSV editing is performed by the app.
  - The backend stores the complete original CSV text on `CsvImportBatch.originalCsv`.
- Raw row preservation:
  - Each parsed CSV data row is stored as a `CsvRawRow`.
  - `CsvRawRow.rawData` stores the original header/value mapping.
  - `CsvRawRow.rawText` stores the raw CSV line text.
  - `CsvRawRow.parsedData` stores normalized parsed values for finalize.
- API endpoints:
  - `POST /api/imports/preview`: create import batch, parse CSV, store raw rows, detect anomalies, return preview.
  - `GET /api/imports/:importId`: fetch a stored preview/report with rows and anomalies.
  - `PATCH /api/imports/:importId/anomalies/:anomalyId`: record user action for an anomaly.
  - `POST /api/imports/:importId/finalize`: import eligible rows, skip ignored rows, block unresolved error rows, generate report.
  - `GET /api/imports/:importId/report`: return report summary, rows, and anomalies.
- Authorization:
  - All import routes require authentication.
  - All import routes require the current user to be a member of the target group or imported batch group.
- Implemented anomaly categories:
  ...
  - `UNKNOWN_PAYER`
  - `UNKNOWN_PARTICIPANT`
  - `EXPENSE_DATE_OUTSIDE_MEMBER_ACTIVE_PERIOD`
  - `DUPLICATE_EXPENSE_ROW`
  - `SAME_EXPENSE_DIFFERENT_AMOUNT`
  - `SETTLEMENT_LOGGED_AS_EXPENSE`
  - `UNSUPPORTED_OR_INCONSISTENT_SPLIT_TYPE`
  - `CURRENCY_MISMATCH`
  - `SPLIT_TOTAL_NOT_MATCHING_EXPENSE_TOTAL`
  - `MEMBER_JOINED_LATER_INCLUDED_IN_OLDER_EXPENSE`
  - `MEMBER_LEFT_EARLIER_INCLUDED_IN_LATER_EXPENSE`
  - `BLANK_NOTES_OR_CATEGORY_AMBIGUITY`
  - `MIXED_DATE_FORMATS`
- User actions supported in preview:
  - `APPROVE_IMPORT`
  - `IGNORE_ROW`
  - `FIXED_EXTERNALLY`
  - `NEEDS_REVIEW`
- Finalization policy:
  - Rows with `IGNORE_ROW` on any anomaly are marked `SKIPPED`.
  - Rows with unresolved `ERROR` anomalies are marked `BLOCKED`.
  - Rows with `NEEDS_REVIEW` on an error anomaly remain `BLOCKED`.
  - Rows without unresolved blocking anomalies are converted into normalized `Expense` and `ExpenseSplit` records.
  - Final split calculation is wrapped during finalize so a bad approved row is blocked instead of crashing the import.
  - Warning anomalies remain visible in the report but do not block import by default.
- Report output:
  - Stored on `CsvImportBatch.report`.
  - Includes import ID, total rows, imported rows, skipped rows, blocked rows, anomaly count, and generation timestamp.
- Current implementation limitations:
  - `expenses_export.csv` is still missing, so exact column mapping and real anomaly counts have not been verified.
  - The preview UI records approve/ignore/fixed/review actions but does not yet provide inline editing of row field values.
  - Settlement-looking rows are detected and blocked from expense import; a dedicated settlement CSV import path is not yet implemented.
  - Currency conversion is intentionally not implemented; non-INR rows are flagged rather than converted.
  - Balance display is still primarily single-currency; imported expenses store `currencyCode`, but multi-currency balance grouping needs follow-up if the CSV contains multiple currencies.
- Verification:
  - `pnpm exec prisma validate` passed.
  - `pnpm exec prisma generate` passed.
  - `pnpm test` passed with 2 test files and 8 tests.
  - `pnpm build` passed for client and server.

## Messy CSV Schema And Explainable Balances Update
- Status: implemented.
- Schema updates:
  - `GroupMember` was renamed to `GroupMembership` with `joinedAt`, `leftAt`, and `membershipType`.
  - `ImportBatch` was renamed to `CsvImportBatch`.
  - `ImportRawRow` was renamed to `CsvRawRow`.
  - `ImportDecision` was added to preserve reviewer decisions as append-only records.
  - `CurrencyRate` was added for explicit exchange-rate correction records.
  - `AuditLog` was added for optional audit events around imports and sensitive actions.
  - `Expense`, `ExpenseSplit`, and `Settlement` now store original currency/minor amounts and normalized INR cents.
- Money rules:
  - Money remains integer minor units.
  - Imported and manual expenses store normalized INR cents used for balance calculation.
  - Original currency and original amount are preserved for auditability.
  - Non-INR import rows continue to be flagged as `CURRENCY_MISMATCH`; no silent conversion is performed.
- CSV safety rules:
  - Original CSV text is stored on `CsvImportBatch`.
  - Original row text and row data are stored on `CsvRawRow`.
  - Suspicious rows are never deleted automatically.
  - Duplicate rows are flagged for approval rather than deleted.
  - Settlement-looking rows are detected as `SETTLEMENT_LOGGED_AS_EXPENSE` and surfaced.
- Balance explanation:
  - API route `GET /api/balances/groups/:groupId/explanation/:userId` returns expense-by-expense and settlement-by-settlement lines for one user.
  - UI route `/groups/:groupId/balances/:userId` shows the selected user's trace, including raw CSV row data when available.
...
- Membership and currency stakeholder requirements:
  ...
  - Sam and Meera: import validation checks membership `joinedAt` and `leftAt` date windows.
- Files changed:
  - `prisma/schema.prisma`
  - `prisma/migrations/20260614020000_explainable_import_schema/migration.sql`
  - `server/src/modules/groups/groupAccess.ts`
  - `server/src/modules/groups/groups.routes.ts`
  - `server/src/modules/expenses/expenses.routes.ts`
  - `server/src/modules/settlements/settlements.routes.ts`
  - `server/src/modules/imports/import.service.ts`
  - `server/src/modules/imports/imports.routes.ts`
  - `server/src/modules/balances/balances.routes.ts`
...
- `GroupMembership`
  - Fields: `id`, `groupId`, `userId`, `joinedAt`, `leftAt`, `membershipType`, `createdAt`.
  - Constraints: unique pair of `groupId` and `userId`.
  - Cascade: deleting a group cascades memberships.
  - Purpose: validates Meera, Sam, Dev, and other members against expense dates during CSV import.
- `Expense`
