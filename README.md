...
| `CLIENT_ORIGIN` | Yes | `https://your-app.example.com` | Public frontend origin allowed by CORS. For multiple frontend URLs, use comma-separated exact origins. In the single-service deployment this should be the same public URL users open. |
| `VITE_API_BASE_URL` | Only for separate frontend deployment | `https://your-api.example.com` | Set on the frontend host before building when the API is deployed on a different origin. Leave empty for the recommended single-service deployment. |
...
- If auth cookies do not persist, verify `NODE_ENV=production`, `CLIENT_ORIGIN` is the exact public HTTPS frontend origin, and the app is served over HTTPS.
- If frontend and backend are deployed separately, set frontend `VITE_API_BASE_URL` to the backend public URL and set backend `CLIENT_ORIGIN` to the frontend public URL. Rebuild/redeploy the frontend after changing `VITE_API_BASE_URL`.
