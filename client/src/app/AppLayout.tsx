...
export function AppLayout() {
  ...
  const [theme, setTheme] = useState(() => localStorage.getItem("equishare-theme") || "light");
  const demoMode = useDemoMode();

  useEffect(() => {
    document.documentElement.dataset.theme = theme;
    localStorage.setItem("equishare-theme", theme);
  }, [theme]);
      ...
      <header className="sticky top-0 z-30 border-b border-line/70 bg-surface/75 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3">
          <Link to="/dashboard" className="group inline-flex items-center gap-3">
            ...
            <span>
              <span className="block text-sm font-bold leading-tight text-ink">Equishare</span>
