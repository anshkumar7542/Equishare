...
export function DashboardPage() {
  ...
  return (
    <div className="grid gap-6">
      <section className="glass-panel hero-panel relative overflow-hidden rounded-lg p-6">
        <div className="grid gap-6 xl:grid-cols-[1.03fr_0.97fr] xl:items-center">
          <motion.div initial={{ opacity: 0, x: -18 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.42, ease: "easeOut" }}>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-line bg-elevated/70 px-3 py-1 text-xs font-semibold text-muted">
              <Sparkles size={14} /> Equishare live dashboard
            </div>
            <h1 className="max-w-4xl text-4xl font-bold tracking-tight text-ink md:text-5xl">
              Hi {user?.name?.split(" ")[0] || "there"}, balance shared money with less back-and-forth.
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">
              Equishare turns group expenses into clear balances, explainable activity, and settlement moves you can trust.
            </p>
          ...
          <motion.div
            className="relative min-h-[280px] overflow-hidden rounded-lg border border-line bg-ink shadow-2xl"
          ...
          >
            ...
            <div className="absolute bottom-4 left-4 right-4 grid gap-3 sm:grid-cols-3">
              <InsightPill label="Owe" value={formatMoney(youOwe)} tone="coral" />
              <InsightPill label="Owed" value={formatMoney(youAreOwed)} tone="mint" />
              <InsightPill label="Net" value={formatMoney(myNet)} tone={myNet >= 0 ? "mint" : "coral"} />
            </div>
          </motion.div>
        </div>
      </section>
...
function HeroOrbit() {
  return (
    <div className="pointer-events-none absolute inset-0">
      <span className="orbit-line absolute left-[14%] top-[18%] h-32 w-32 rounded-full border border-white/20" />
      <span className="float-chip absolute right-[18%] top-[15%] rounded-md border border-white/20 bg-white/90 px-3 py-2 text-xs font-bold text-ink shadow-xl">
        Settled
      </span>
      <span className="float-chip-delayed absolute left-[18%] top-[45%] rounded-md border border-white/20 bg-white/90 px-3 py-2 text-xs font-bold text-ink shadow-xl">
        Matched
      </span>
    </div>
  );
...
function InsightPill({ label, value, tone }: { label: string; value: string; tone: "mint" | "coral" }) {
  const toneClass = tone === "mint" ? "text-mint" : "text-coral";
  return (
    <div className="rounded-md border border-white/15 bg-white/90 p-3 shadow-xl backdrop-blur">
